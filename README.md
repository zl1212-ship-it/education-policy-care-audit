...
2. **Survival to Study Ratio ($R_{ss}$):** Models temporal precarity thresholds.
   $$R_{ss} = \frac{T_{survival}}{T_{research}}$$
   Where $T_{survival}$ denotes the hours forced into secondary subsistence labor, and $T_{research}$ signifies active hours dedicated to scholarly investigation.
