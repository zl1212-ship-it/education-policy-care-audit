...
PROVISIONS = {
    ...
    "prohibition": re.compile(
        ...
        r"\bban(?:ned|s)?\b|unauthori[sz]ed use|impermissible|disallow", re.I),
    "detector_surveillance": re.compile(
        r"\bdetector\b|\bdetection\b|turnitin|gptzero|originality report|"
        r"ai[\s-]?detection|detection (?:tool|software)|plagiarism detect|"
        r"text[\s-]?match|similarity (?:report|score|index)|urkund|ouriginal", re.I),
    "misconduct_framing": re.compile(
        r"plagiar|academic misconduct|academic dishonest|integrity violation|"
        r"cheating|unauthorized assistance|misrepresent|"
        r"collusion|academic offen[cs]e|integrity breach", re.I),
    "sanction": re.compile(
        r"sanction|penalt|failing grade|fail the (?:course|assignment|class|unit)|"
        ...
        r"dismissal|exclusion|excluded from|deduction of marks|mark of zero|academic penalty", re.I),
    ...
    "disclosure": re.compile(
        r"disclos|acknowledg(?:e|es|ed|ing|ement|ment)\s+(?:the\s+|any\s+|their\s+|its\s+|your\s+)?use|"
    ...
    "appeal": re.compile(
        ...
        r"right to respond|due process|reconsideration|student complaint|"
        r"review of the (?:decision|finding)", re.I),
    "l2_protection": re.compile(
        r"english language learner|multilingual|non[\s-]?native|first language|"
        r"\besl\b|\beal\b|second[\s-]?language|english as an? (?:second|additional)|"
        r"english as an additional language|international student", re.I),
}
